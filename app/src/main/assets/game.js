    ...
    create() {
        ...
        setTimeout(() => {
            for (let i = 0; i < g.number; i++)  {
                g.circle = this.add.circle(Math.random() * config.width,
                Math.random() * config.height,5,randomColor())
         ...
         setTimeout(() => {
            for (let i = 0; i < g.number; i++)  {
                g.circle = this.add.circle(Math.random() * config.width,
                Math.random() * config.height,10,randomColor())
                this.physics.add.existing(g.circle)
                g.circle.body.setVelocity(randomVelocity(), randomVelocity())
                g.circle.body.setCollideWorldBounds(true, 1, 1)
                g.arr.push(g.circle)
        ...
        setTimeout(() => {
            for (let i = 0; i < g.number; i++)  {
                g.circle = this.add.circle(Math.random() * config.width,
                Math.random() * config.height,15,randomColor())
                this.physics.add.existing(g.circle)
                g.circle.body.setVelocity(randomVelocity(), randomVelocity())
                g.circle.body.setCollideWorldBounds(true, 1, 1)
                g.arr.push(g.circle)
